...
const Card = ({ day, index, unit }) => {
  ...
  const unitShorthard = unit === "kelvin" ? unit.slice(0, 1).toUpperCase() : "°" + unit.slice(0, 1).toUpperCase()
  ...
  return (
    <div className="card-container">
      ...
      <p>{day.weather}</p>
      <p>{convertUnit(unit, day.temp2m.max)} {unitShorthard} max</p>
      <p>{convertUnit(unit, day.temp2m.min)} {unitShorthard} min</p>
    </div>
  )
}
